...
public interface ApiVersionMasterRepository extends
		JpaRepository<ApiVersionMaster, Integer> {

	// public List<ApiVersionMaster> findAllFetchAll();

	@Query("select versionmaster from ApiVersionMaster versionmaster left join fetch versionmaster.apiRootUrlMasters master left join fetch master.apiInformations info left join fetch info.apiMediaTypes left join fetch info.requestParams left join fetch info.returnParams left join fetch info.errorCodes   where versionmaster.id = ?1 ")
	public ApiVersionMaster findByIdFetchAll(Integer id);

	// info left join fetch info.requestParamList left join fetch
	// info.returnParamList left join fetch info.apiMediaTypeSet
	@Query("select versionmaster from ApiVersionMaster versionmaster left join fetch versionmaster.apiRootUrlMasters master left join fetch master.apiInformations info left join fetch info.apiMediaTypes left join fetch info.requestParams left join fetch info.returnParams left join fetch info.errorCodes  where versionmaster.version = ?1 and versionmaster.apiMaster.id = ?2")
	...
	public ApiVersionMaster findByVersionAndApiMasterIdFetchAll(
			Integer version, Integer apiMasterId);

	public ApiVersionMaster findById(Integer id);

	@Query("select master from ApiVersionMaster master left join fetch master.apiRootUrlMasters where master.id = ?1")
	public ApiVersionMaster findByIdFetchApiRootUrlMasters(Integer id);
}
