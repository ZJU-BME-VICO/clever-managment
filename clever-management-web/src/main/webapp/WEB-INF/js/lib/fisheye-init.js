if (typeof window.jQuery != "undefined") {
  ...
  jQuery.fn = jQuery.prototype = {
    ...
    setArray: function(a) {
      this.length = 0; [].push.apply(this, a);
      return this
    },
    ...
    filter: function(t) {
      return this.pushStack(jQuery.isFunction(t) && jQuery.grep(this,
      function(el, index) {
        return t.apply(el, [index])
      }) || jQuery.multiFilter(t, this))
    },
    not: function(t) {
      return this.pushStack(t.constructor == String && jQuery.multiFilter(t, this, true) || jQuery.grep(this,
      function(a) {
  ...
  jQuery.extend = jQuery.fn.extend = function() {
    ...
    var prop;
    while (prop = arguments[a++]) for (var i in prop) target[i] = prop[i];
    return target
  };
  jQuery.extend({
    noConflict: function() {
    ...
    each: function(obj, fn, args) {
      if (obj.length == undefined) for (var i in obj) fn.apply(obj[i], args || [i, obj[i]]);
      else for (var i = 0,
      ol = obj.length; i < ol; i++) if (fn.apply(obj[i], args || [i, obj[i]]) === false) break;
      return obj
    },
      ...
      if (p == "height" || p == "width") {
        ...
        function() {
          if (jQuery.css(e, "display") != "none") {
            ...
            if (parPos == "" || parPos == "static") e.parentNode.style.position = "static";
    ...
    clean: function(a) {
      ...
      function(i, arg) {
        ...
        if (arg.length === 0 && !jQuery.nodeName(arg, "form")) return;
    ...
    attr: function(elem, name, value) {
      ...
      } else if (name == "opacity" && jQuery.browser.msie) return elem.filter ? parseFloat(elem.filter.match(/alpha\(opacity=(.*)\)/)[1]) / 100 : 1;
    ...
    makeArray: function(a) {
      var r = [];
      if (a.constructor != Array) for (var i = 0,
      al = a.length; i < al; i++) r.push(a[i]);
      else r = a.slice(0);
      return r
    },
    inArray: function(b, a) {
      for (var i = 0,
      al = a.length; i < al; i++) if (a[i] == b) return i;
      return - 1
    },
    merge: function(first, second) {
      var r = [].slice.call(first, 0);
      for (var i = 0,
      sl = second.length; i < sl; i++) if (jQuery.inArray(second[i], r) == -1) first.push(second[i]);
      return first
    },
    grep: function(elems, fn, inv) {
      if (typeof fn == "string") fn = new Function("a", "i", "return " + fn);
      var result = [];
      for (var i = 0,
      el = elems.length; i < el; i++) if (!inv && fn(elems[i], i) || inv && !fn(elems[i], i)) result.push(elems[i]);
      return result
    },
    map: function(elems, fn) {
      ...
      check: for (var i = 1,
      rl = result.length; i < rl; i++) {
        for (var j = 0; j < i; j++) if (result[i] == r[j]) continue check;
        r.push(result[i])
      }
      return r
    }
  ...
  function(i, n) {
    jQuery.fn[i] = function() {
      ...
      return this.each(function() {
        for (var j = 0,
        al = a.length; j < al; j++) jQuery(a[j])[n](this)
      })
    }
  });
  jQuery.each({
  ...
  jQuery.extend({
    ...
    parse: [/^\[ *(@)([a-z0-9_-]*) *([!*$^=]*) *('?"?)(.*?)\4 *\]/i, /^(\[)\s*(.*?(\[.*?\])?[^[]*?)\s*\]/, /^(:)([a-z0-9_-]*)\("?'?(.*?(\(.*?\))?[^(]*?)"?'?\)/i, /^([:.#]*)([a-z0-9_*-]*)/i],
    token: [/^(\/?\.\.)/, "a.parentNode", /^(>|\/)/, "jQuery.sibling(a.firstChild)", /^(\+)/, "jQuery.nth(a,2,'nextSibling')", /^(~)/,
    function(a) {
    ...
    find: function(t, context) {
      ...
      while (t && last != t) {
        ...
        if (m) {
          jQuery.each(ret,
          function() {
            for (var c = this.firstChild; c; c = c.nextSibling) if (c.nodeType == 1 && (jQuery.nodeName(c, m[1]) || m[1] == "*")) r.push(c)
          });
          ret = r;
          t = t.replace(re, "");
        ...
        if (t && !foundToken) {
          if (!t.indexOf(",")) {
            ...
            if (m[1] == "#" && ret[ret.length - 1].getElementById) {
              ...
              if (m[1] == ".") var rec = new RegExp("(^|\\s)" + m[2] + "(\\s|$)");
              jQuery.each(ret,
              function() {
                var tag = m[1] != "" || m[0] == "" ? "*": m[2];
              ...
              if (m[1] == "." && ret.length == 1) r = jQuery.grep(r,
              function(e) {
    ...
    getAll: function(o, r, token, name, re) {
      for (var s = o.firstChild; s; s = s.nextSibling) if (s.nodeType == 1) {
        var add = true;
        if (token == ".") add = s.className && re.test(s.className);
        else if (token == "#") add = s.getAttribute("id") == name;
        if (add) r.push(s);
        if (token == "#" && r.length) break;
        if (s.firstChild) jQuery.getAll(s, r, token, name, re)
      }
      return r
    },
    ...
    nth: function(cur, result, dir, elem) {
      ...
      for (; cur; cur = cur[dir]) {
        ...
        if (num == result || result == "even" && num % 2 == 0 && num > 1 && cur == elem || result == "odd" && num % 2 == 1 && cur == elem) return cur
      }
    },
  ...
  jQuery.event = {
    ...
    remove: function(element, type, handler) {
      if (element.$events) {
        ...
        else for (i in element.$events[type]) delete element.$events[type][i];
        else for (j in element.$events) this.remove(element, j);
        for (k in element.$events[type]) if (k) {
          k = true;
          break
        }
    ...
    trigger: function(type, data, element) {
      ...
      if (!element) jQuery.each(this.global[type] || [],
      function() {
        ...
        if (handler) {
          ...
          if ((val = handler.apply(element, data)) !== false) this.triggered = true
        }
        if (fn && val !== false) element[type]();
  ...
  function() {
    ...
    for (var type in global) {
      var els = global[type],
      i = els.length;
      if (i && type != 'unload') do jQuery.event.remove(els[i - 1], type);
      while (--i)
    }
  });
  jQuery.fn.extend({
    ...
    load: function(url, params, callback, ifModified) {
      ...
      jQuery.ajax({
        ...
        complete: function(res, status) {
          if (status == "success" || !ifModified && status == "notmodified") self.attr("innerHTML", res.responseText).evalScripts().each(callback, [res.responseText, status, res]);
          else callback.apply(self, [res.responseText, status, res])
        }
      });
      return this
    },
  ...
  jQuery.fn.extend({
    show: function(speed, callback) {
      ...
      speed ? hidden.animate({
        height: "show",
        width: "show",
        opacity: "show"
      },
      speed, callback) : hidden.each(function() {
    ...
    hide: function(speed, callback) {
      ...
      speed ? visible.animate({
        height: "hide",
        width: "hide",
        opacity: "hide"
      },
      speed, callback) : visible.each(function() {
  ...
  jQuery.extend({
    ...
    fx: function(elem, options, prop) {
      ...
      z.a = function() {
        ...
        if (prop == "opacity") jQuery.attr(y, "opacity", z.now);
      ...
      z.step = function(firstNum, lastNum) {
        ...
        if (t > options.duration + z.startTime) {
          ...
          var done = true;
          for (var i in elem.curAnim) if (elem.curAnim[i] !== true) done = false;
          if (done) {
            y.overflow = "";
            y.display = oldDisplay;
            if (jQuery.css(elem, "display") == "none") y.display = "block";
            if (options.hide) y.display = "none";
            if (options.hide || options.show) for (var p in elem.curAnim) if (p == "opacity") jQuery.attr(y, p, elem.orig[p]);
            else y[p] = ""
          }
          ...
          z.now = options.easing && jQuery.easing[options.easing] ? jQuery.easing[options.easing](p, n, firstNum, (lastNum - firstNum), options.duration) : (( - Math.cos(p * Math.PI) / 2) + 0.5) * (lastNum - firstNum) + firstNum;
          z.a()
        }
      }
    }
