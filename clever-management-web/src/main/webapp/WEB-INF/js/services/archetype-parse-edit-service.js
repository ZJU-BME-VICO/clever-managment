...
angular.module('clever.management.service.archetypeParseEdit', []).service('archetypeParseEditService', function(rmFactoryService) {
     ...
     * @Author Mecro
     * Param
     *       typeNode : the ndoe to be process
             parent : the parent of typeNode
             treeItems : tree items to wrap the constructed node
             terminologies : ontology part 
             parentAttribute : for some node, the attribute should not be displaied, but we need to access the parent of this node, so a parent attribute is needed
      *@Function process the node which is a CComplexObject
      */
