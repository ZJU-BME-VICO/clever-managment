				...
				function(rmFactoryService) {
					...
					this.parseTerminology = function(archetype) {
						...
						return {
							...
							// editable array,
							oriNodeRef : archetype.terminology,
						...
						if (!parentNode) {
							...
							if (node.rm_attribute_name) {
								...
								if (node.node_id) {
									...
									node.textPath = parentNode.textPath
											+ "["
											+ getOntologyByNodeId(node.node_id,
													"en", termDefinitions)
											+ "]";
					...
					function processSingleNode(value, parent, treeItems,
						...
						var rmObject = rmFactoryService[value.rm_type_name]
								&& new rmFactoryService[value.rm_type_name];
						var keepAttribute = rmObject
								&& rmObject.menuType == 'ATTRIBUTE';
					...
					function processSingleAttribute(attribute, parent,
						...
						if (keepAttribute) { // if attribute type should be
							// keep
							...
							if (node.attributes) {
								...
								if (angular.isArray(nodeAttributes)) {
									...
									for (var i = 0; i < node.attributes.length - 1; i++) {
										if (node.attributes[i].rm_attribute_name == "name") { // find
											// a
											// attribute
											// which
											// name
									...
									if (count != undefined) {
										if (node.attributes[count].children.attributes) { // if
											// there
											// is a
											// attribute
											// which
