function ArchetypeEditCtrl($scope, $modal, $log, $q, $timeout, msgboxService,
		busyService, archetypeEditService, documentDiffModalService,
		resourceService, archetypeParseEditService, templateParseToEditService,
		archetypeSerializeService, archetypeParseService,
		treeDataFormatService, ARCHETYPE_BY_NAME_URL,
		ARCHETYPE_LIST_EDIT_DRAFT_URL, ARCHETYPE_LIST_EDIT_PUBLISHED_URL,
		ARCHETYPE_CREATE_BY_URL, AECHETYPE_FALLBACK_BY_ID_URL,
	 ...
	 * 
	 * @param {object}
	 *            node An archetype node in archetype tree
	 * @return {string} Node name if node is a directory, node conceptName with
	 *         node latestArchetypeVersion if node is a archetype
	 */
	...
	$scope.loadArchetypeList = function() {
		...
		var busyId = busyService.pushBusy('BUSY_LOADING');
		resourceService.get(ARCHETYPE_LIST_EDIT_PUBLISHED_URL).then(
				function(list) {
	...
	/**
	 * Concat draft and published archetype list, use formatTreeData service
	 * generate a foramted tree data which can be used as tree view data, if
	 * archetype name need located is not null ,locate this archetype.
	 */
	function generateTreeData() {
		if ($scope.draftLoadOver && $scope.publishedLoadOver) {
			$scope.draftLoadOver = false;
			$scope.publishedLoadOver = false;
			var list = $scope.draftArchetypeList
					.concat($scope.publishedArchetypeList);
			$scope.originalArchetypeList = list;

			$scope.formatedObject = treeDataFormatService.formatTreeData(list,
					'specialiseArchetype');
			...
			if (name) {
				...
				$timeout(function() {
					$scope.locateArchetype(findArchetypeByName(
							$scope.originalArchetypeList, name));
	 ...
	 * @param {string}
	 *            name Archetype id
	 * @return {object | undefined} A archetype object with archetype id equals
	 *         to name or undefined if find failure
	 */
	 ...
	 * 
	 * @param {array}
	 *            list An archetype list
	 */
	function formatArchetypeSpecialiseRelation(list) {
		if (angular.isArray(list)) {
			angular.forEach(list, function(archetype) {
				if ((!archetype.specialiseArchetype)
						|| (!archetype.specialiseArchetype.adl)) {
	...
	/**
	 * local api for locateNode in tree view directive, find and hightlight the
	 * archetype node in archetype tree
	 * 
	 * @param {object}
	 *            arc An archetype node object in archetype tree
	 */
	 ...
	 * 
	 * @param {string}
	 *            title Archetype name string
	 * @param {length}
	 *            length Max archetype name length
	 * @return {string} return title if title length is lower than length, or
	 *         return a shorten one with '...' inside
	 * 
	 */
	$scope.getFixedTitle = function(title, length) {
		if (title) {
			...
			if (title.length > titleLength) {
				return title.substring(0, titleLength / 2)
						+ '...'
						+ title.substring(title.length - titleLength / 2,
								title.length);
			} else
				return title;
		}
	...
	/**
	 * callback function for click tree view node, select node if the selected
	 * archetype is undefined, or check original adl and current adl, if there
	 * is difference between them, alert a save ask dialog, or select node
	 * 
	 * @param {object}
	 *            node archetype node in tree view
	 */
	...
	function originalEqualToEdited() {
		var currentSmAdl = trim(archetypeSerializeService
				.serializeArchetype($scope.oriArchetype));
	...
	function alertSaveAskDialog(node) {
		msgboxService.createMessageBox("OPERATE_PROMPT_HINT",
				"ARCHETYPE_EDIT_SAVEASK_HINT", {
					archetypeName : $scope.name
	...
	/**
	 * Callback function for click archetype node in archetype tree, do nothing
	 * if node is a directory, or: collapse definition tree, assign archetype to
	 * selectedArchetype, save original adl for generating difference initialize
	 * originial Archetype and result json, the result json is used to display
	 * especially in definition pane and original archetype is the target of the
	 * editor, all change should apply to original archetype and it will be
	 * serialized to adl when a save operate is executed.
	 */
	var selectArchetype = function(archetype) {
		...
		if (archetype.isDirectory) {
			...
			resourceService
					.get(ARCHETYPE_BY_NAME_URL + archetype.name + '/adl').then(
							function(result) {
								if (result) {
			...
			var bid = busyService.pushBusy('BUSY_LOADING');
			resourceService
					.get(ARCHETYPE_BY_NAME_URL + archetype.name + '/xml')
					.then(
							function(result) {
								if (result) {
									...
									busyService.popBusy(bid);
									var busyId = busyService
											.pushBusy('BUSY_LOADING');
									$q(
											function(resolve, reject) {
												setTimeout(
														function() {
															var oriArchetype = archetypeParseEditService
																	.getOriginalArchetype($scope.selectedArchetype.xml);
															var resultJson = archetypeParseEditService
																	.parseArchetypeJson(oriArchetype);
															if (oriArchetype
																	&& resultJson) {
																resolve({
											...
											.then(
													function(data) {
														...
														console
																.log('this is archetype object after parse (form xml to json)');
														console
																.log(oriArchetype);
														console
																.log('this is archetype object after parse to edit (form json to json)');
														...
														if (angular
																.isArray(languages)) {
															$scope.languages.originalLanguage = languages
																	.find(function(
																			value) {
	...
	/**
	 * callback function for display tab clicked, serialize the original
	 * archetype to adl and xml to display the edit result
	 */
	$scope.refreshAdlAndXml = function() {
		...
		$q(
				function(resolve, reject) {
					setTimeout(
							function() {
								var adl = archetypeSerializeService
										.serializeArchetype($scope.oriArchetype);
								var xml = archetypeSerializeService
										.serializeArchetypeToXml($scope.oriArchetype);
								if (adl) {
									resolve({
										adl : adl,
										xml : xml,
	...
	/**
	 * parse xml to json object and process its archetypeId to a new reivision
	 * one (eg. v1.1 to v1.2) push the result json to archetype list, the
	 * pushToArchetypeList will process the remaining work
	 * 
	 * @param {object}
	 *            archetype The selected archetype which will be process and
	 *            generate a new one
	 */
	function createNewRevisionArchetype(archetype) {
		var jsonArchetype = archetypeParseEditService
				.getOriginalArchetype(archetype.xml);
		var archetypeId = jsonArchetype.archetype_id.value;
		var frontString = archetypeId.slice(0,
				archetypeId.lastIndexOf('v1') + 3);
		var lastString = archetypeId.slice(archetypeId.lastIndexOf('v1') + 3,
				archetypeId.length);
	...
	/**
	 * upsize the revision. get revision string and parse it to Int, upsize the
	 * result and add it to body string
	 * 
	 * @param {string}
	 *            archetypeId
	 */
	function upsizingRevision(archetypeId) {
		...
		// get revision string
		var revisionString = archetypeId
				.slice(dotIndex + 1, archetypeId.length);
	...
	/**
	 * upsize the version. get version string and parse it to Int, replace the
	 * number in original string.
	 * 
	 * @param {object}
	 *            archetypeId
	 */
	function upsizingVersion(archetypeId) {
		...
		var version = parseInt(versionMatchInfo[0].slice(1,
				versionMatchInfo[0].length));
	...
	/**
	 * push a element to an array, if array is not an array, create a new one,
	 * and push the array and node into it.
	 * 
	 * @param {object}
	 *            node Any type element
	 * @param {object}
	 *            array May be an array or an Obejct
	...
	function addActivityToInstruction(instruction) {
		var ItemTree = editor.getCComplexObject(undefined, "at0002", editor
				.getDefaultOccurrences(1, 1), "ITEM_TREE");
		var description = editor.getCSingleAttribute(ItemTree, editor
				.getDefaultExistence(1, 1), "description");
		var activityObject = editor.getCComplexObject(description, "at0001",
				editor.getDefaultOccurrences(0, 1), "ACTIVITY");
		var activitiesAttribute = editor.getCMultipleAttribute(activityObject,
				editor.getDefaultCardinality(0), editor.getDefaultExistence(1,
						1), "activities");

		instruction.definition.attributes = pushTo(activitiesAttribute,
				instruction.definition.attributes);

		editor.synchronizeOriOntology("at0002", "Tree", "@internal@",
				instruction.ontology);
		editor.synchronizeOriOntology("at0001", "Current Activity",
				"Current Activity", instruction.ontology);
	}
	...
	function getEvent() {
		var ItemTree = editor.getCComplexObject(undefined, "at0003", editor
				.getDefaultOccurrences(1, 1), "ITEM_TREE");
		var dataAttribute = editor.getCSingleAttribute(ItemTree, editor
				.getDefaultExistence(1, 1), "data");
		var eventObject = editor.getCComplexObject(dataAttribute, "at0002",
				editor.getDefaultOccurrences(0, 1), "EVENT");
		return eventObject;
	}

	function addDataToObservation(observation) {
		var eventObject = getEvent();
		var eventsAttribute = editor.getCMultipleAttribute(eventObject, editor
				.getDefaultCardinality(1), editor.getDefaultExistence(1, 1),
				"events");
		var historyObject = editor.getCComplexObject(eventsAttribute, "at0001",
				editor.getDefaultOccurrences(1, 1), "HISTORY");
		var dataAttribute = editor.getCSingleAttribute(historyObject, editor
				.getDefaultExistence(1, 1), "data");
		// observation.definition.attributes.push(dataAttribute);
		observation.definition.attributes = pushTo(dataAttribute,
				observation.definition.attributes);
		editor.synchronizeOriOntology("at0001", "Event Series", "@ internal @",
				observation.ontology);
		editor.synchronizeOriOntology("at0002", "Any event", "*",
				observation.ontology);
		editor.synchronizeOriOntology("at0003", "Tree", "@ internal @",
				observation.ontology);
	}
	// end
	...
	function createEvaluation(info) {
		...
		// add default element here
		var itemTree = editor.getCComplexObject(undefined, "at0001", editor
				.getDefaultOccurrences(1, 1), "ITEM_TREE");
		editor.synchronizeOriOntology("at0001", "Tree", "@ internal @",
				jsonEvaluation.ontology);
		var dataAttribute = editor.getCSingleAttribute(itemTree, editor
				.getDefaultExistence(1, 1), "data");
		// jsonEvaluation.definition.attributes.push(dataAttribute);
		jsonEvaluation.definition.attributes = pushTo(dataAttribute,
				jsonEvaluation.definition.attributes);
	...
	function createAdminEntry(info) {
		...
		var ItemTree = editor.getCComplexObject(undefined, "at0001", editor
				.getDefaultOccurrences(1, 1), "ITEM_TREE");
		var dataAttribut = editor.getCSingleAttribute(ItemTree, editor
				.getDefaultExistence(1, 1), "data");
		// jsonAdminEntry.definition.attributes.push(dataAttribut);
		jsonAdminEntry.definition.attributes = pushTo(dataAttribut,
				jsonAdminEntry.definition.attributes);
		// synchronize the ontology
		editor.synchronizeOriOntology("at0001", "Tree", "@ internal @",
				jsonAdminEntry.ontology);
	...
	function createComposition(info) {
		...
		var codePhrase = editor.getCCodePhraseWithPara("openehr", [ '433' ],
				undefined);
		var definingCode = editor.getCSingleAttribute(codePhrase, editor
				.getDefaultExistence(1, 1), "defining_code");
		var codeText = editor.getCComplexObject(definingCode, undefined, editor
				.getDefaultOccurrences(1, 1), "DV_CODED_TEXT");
		var category = editor.getCSingleAttribute(codeText, editor
				.getDefaultExistence(1, 1), "category");

		// jsonCompositon.definition.attributes.push(category);
		jsonCompositon.definition.attributes = pushTo(category,
				jsonCompositon.definition.attributes);
	...
	function createItemTable(info) { // not absolutely right
		...
		var rotated = editor.getCSingleAttribute([], editor
				.getDefaultExistence(1, 1), "rotated");
		var cluster = editor.getCComplexObject([], "at0002", editor
				.getDefaultOccurrences(0, 1), "CLUSTER");
		var rows = editor.getCMultipleAttribute(cluster, editor
				.getDefaultCardinality(1), editor.getDefaultExistence(1, 1),
				"rows");

		// jsonItemTable.definition.attributes.push(rows);
		jsonItemTable.definition.attributes = pushTo(rows,
				jsonItemTable.definition.attributes);
		jsonItemTable.definition.attributes.push(rotated);

		editor.synchronizeOriOntology("at0002", "row", "@interval@",
				jsonItemTable.ontology);
		editor.synchronizeOriOntology("at0001", "Table", "@interval@",
				jsonItemTable.ontology);
	...
	function pushToArchetypeList(jsonObj) {
		...
		console.log('this is the adl of the created archetype!')
		console.log(adl);
		var bid = busyService.pushBusy('BUSY_LOADING');

		resourceService
				.post(ARCHETYPE_CREATE_BY_URL, {
					adl : adl,
				})
				.then(
						function(result) {
							...
							if (result.succeeded) {
								...
								$scope.loadArchetypeList();
								msgboxService.createMessageBox(
										'ARCHETYPE_EDIT_SUCCEEDED',
										'ARCHETYPE_EDIT_ADD_SUCCEEDED_HINT',
										{}, 'success');
							} else {
								msgboxService.createMessageBox(
										'ARCHETYPE_EDIT_FAILED',
										'ARCHETYPE_EDIT_ADD_FAILED_HINT', {
	...
	function getArchetypeId(info) {
		return info.organization + "-" + info.referenceModel + '-'
				+ info.entityType + "." + info.concept + ".v1.1";
	}
	...
	var ehrEntity = [ "OBSERVATION", "EVALUATION", "INSTRUCTION",
			"COMPOSITION", "ADMIN_ENTRY", "ACTION", "ELEMENT", "ITEM_LIST",
			"ITEM_SINGLE", "ITEM_TREE", "ITEM_TABLE", "CLUSTER", "SECTION" ];
	var demograEntity = [ 'ADDRESS', 'AGENT', 'CAPABILITY', 'CLUSTER',
			'CONTACT', 'ELEMENT', 'GROUP', 'ITEM_LIST', 'ITEM_SINGLE',
			'ITEM_TABLE', 'ITEM_TREE', 'ORGANISATION', 'PARTY_IDENTIFIED',
			'PARTY_RELATIONSHIP', 'PERSON', 'ROLE' ];
	...
	$scope.openCreate = function() {
		var modalInstance = $modal.open({
			...
			controller : function ArchetypeCreateCtrl($scope, $modalInstance,
					organizations, referenceModels, entityMap) {
	...
	var specialiseArchetype = function(value, conceptName) {
		...
		var lastString = tempString.slice(tempString.indexOf('.'),
				tempString.length);
	...
	$scope.openSpecialise = function(size) {
		var modalInstance = $modal.open({
			...
			controller : function ArchetypeSpecialiseCtrl($scope,
					$modalInstance, specialisingArchetypeName) {
		...
		modalInstance.result.then(function(message) {
			specialiseArchetype($scope.specialisedArchetype,
					message.newConceptName);
	...
	/**
	 * function open the difference modal, check the difference between edited
	 * archetype and original archetype
	 */
	$scope.generateDiff = function() {
		var editedArchetype = archetypeSerializeService
				.serializeArchetype($scope.oriArchetype);
		documentDiffModalService.open('Modify records', editedArchetype,
				$scope.selectedArchetype.adl);
	};
	...
	$scope.submitSelectedArchetype = function() {
		var bid = busyService.pushBusy('BUSY_LOADING');
		var archetype = archetypeSerializeService
				.serializeArchetype($scope.oriArchetype);
		resourceService.get(
				ARCHETYPE_SUBMIT_BY_ID_URL + $scope.selectedArchetype.id).then(
				function(result) {
					busyService.popBusy(bid);
					if (result.succeeded) {
						...
						msgboxService.createMessageBox(
								'ARCHETYPE_EDIT_SUCCEEDED',
								'ARCHETYPE_EDIT_SUBMIT_SUCCEEDED_HINT', {},
								'success');
					} else {
						msgboxService.createMessageBox('ARCHETYPE_EDIT_FAILED',
								'ARCHETYPE_EDIT_SUBMIT_FAILED_HINT', {
									errorMsg : result.message
								}, "error");
					}
				});
	...
	function saveSelectedArchetype() {
		...
		console.log(adl);
		return resourceService.post(ARCHETYPE_EDIT_BY_ID_URL
				+ $scope.selectedArchetype.id, {
			adl : adl,
		});
	}
	...
	function processSaveResult(result) {
		if (result.succeeded) {
			msgboxService.createMessageBox('ARCHETYPE_EDIT_SUCCEEDED',
					'ARCHETYPE_EDIT_SAVE_SUCCEEDED_HINT', {}, 'success');
		} else {
			msgboxService.createMessageBox('ARCHETYPE_EDIT_FAILED',
					'ARCHETYPE_EDIT_SAVE_FAILED_HINT', {
	...
	$scope.fallbackArchetype = function() {
		var bid = busyService.pushBusy('BUSY_LOADING');
		resourceService
				.get(AECHETYPE_FALLBACK_BY_ID_URL + $scope.selectedArchetype.id)
				.then(
						function(result) {
							busyService.popBusy(bid);
							if (result.succeeded) {
								msgboxService.createMessageBox(
										'ARCHETYPE_EDIT_SUCCEEDED',
										'ARCHETYPE_FALLBACK_SUCCEEDE_HINT', {},
										'success');
								$scope.selectedArchetype.lifecycleState = "Teamreview";
							} else {
								msgboxService.createMessageBox(
										'ARCHETYPE_EDIT_FAILED',
										'ARCHETYPE_FALLBACK_FAILED_HINT', {
											errorMsg : result.message
										}, "error");
							}
						});
